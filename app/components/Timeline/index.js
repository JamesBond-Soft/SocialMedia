...
import { addMedia, deleteMedia, favoritePost, unFavoritePost } from '../../redux/actions/timeline';
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <div className={Styles.mediaDate}>{Props.item.postDate}</div>
        {
           Props.item.favorited ?
             <div onClick={Props.unFavoritePost} className={Styles.favoriteBtn}>UnFavorite</div> :
             <div onClick={Props.favoritePost} className={Styles.favoriteBtn}>Favorite</div>
         }
      </div>
      <div>
...
class Timeline extends Component {
  ...
  */
  ...
  /**
   * Handle for make post favorite
   */
  favoritePost = (item) => (e) => {
    const id = item.id;
    console.log('------------ favorite is clicked ----------- ', id);
    this.props.dispatch(favoritePost(id));
  }

  unFavoritePost = (item) => (e) => {
    const id = item.id;
    console.log('------------ favorite is clicked ----------- ', id);
    this.props.dispatch(unFavoritePost(id));
  }

  switchFavorite = () => {
    this.setState(prevState => ({
      showFavoriteLists: !prevState.showFavoriteLists
    }));
  }
    ...
    if (filterList) {
      return (
        filterList.map((item, index) => (
          <MediaItem {...this.state} favoritePost={this.favoritePost(item)} unFavoritePost={this.unFavoritePost(item)} customClickEvent={this.selectItem(item)} key={index} item={item} />
        ))
      );
    }
  }
